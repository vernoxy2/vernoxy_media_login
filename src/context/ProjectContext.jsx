...
export function ProjectProvider({ children }) {
  ...
  const addProject = async (projectData) => {
    try {
      ...
      // ✅ FIX: Create initial userTask if assignedTo is provided and userTasks don't exist
      let userTasks = projectData.userTasks || [];
      
      if (projectData.assignedTo && userTasks.length === 0) {
        const assignedMember = teamMembers.find(m => m.id === projectData.assignedTo);
        
        const initialUserTask = {
          userId: projectData.assignedTo,
          userEmail: assignedMember?.email || "",
          userName: assignedMember?.name || "",
          taskStatus: "pending",
          startTime: null,
          endTime: null,
          estimatedHours: projectData.estimatedHours || "0",
          estimatedMinutes: projectData.estimatedMinutes || "0",
          timeLog: [],
        };
      ...
      const newProject = {
        ...projectData,
        userTasks: userTasks, // ✅ Include userTasks
        createdAt: serverTimestamp(),
        updatedAt: serverTimestamp(),
        createdBy: currentUser.email,
      };
      ...
      const addedProject = {
        ...projectData,
        id: docRef.id,
        userTasks: userTasks, // ✅ Include in returned data
        createdAt: new Date(),
        updatedAt: new Date(),
        createdBy: currentUser.email,
      };
